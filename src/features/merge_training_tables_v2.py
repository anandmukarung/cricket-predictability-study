...
PLAYER_TABLE_PATH = Path("data/processed/training_table_player_v2.parquet")
VENUE_TABLE_PATH = Path("data/processed/training_table_venue_v1.parquet")

OUTPUT_DIR = Path("data/processed")
OUTPUT_CSV = OUTPUT_DIR / "training_table_v2.csv"
OUTPUT_PARQUET = OUTPUT_DIR / "training_table_v2.parquet"
...
def trim_feature_table(df: pd.DataFrame, keep_match_id_only_from_base: bool = True) -> pd.DataFrame:
    """
    Keep match_id plus non-base feature columns so merges don't create duplicate metadata columns.
    """
...
def main() -> None:
    ...
    player_df = load_table(PLAYER_TABLE_PATH, "player table")
    ...
    player_features = trim_feature_table(player_df)
    ...
    # If available, keep only binary-outcome matches
